...
const uniformModes = [
  {
    ...
    slug: "after-dark",
    name: "After Dark",
    signal: "Low light. High impact.",
    tag: "USL-26-SS-AFDRK",
    environment: "Low light",
    image: "/assets/unusual-drop-outerwear.png",
    alt: "Model in a black UNUSUAL technical shell styled as a full after dark uniform.",
    position: "50% 28%",
    pieces: [
      {
        number: "01",
        ...
        price: 268,
        image: "/assets/unusual-drop-outerwear.png",
        alt: "Black tactical coat with UNUSUAL branding.",
        position: "50% 30%",
      },
      {
        number: "02",
      ...
      },
      {
        ...
        price: 88,
        image: "/assets/unusual-drop-accessories.png",
        alt: "Black utility crossbody bag styled with technical layers.",
        position: "50% 34%",
      },
      {
        number: "04",
  ...
  },
  {
    ...
    slug: "match-day",
    name: "Match Day",
    signal: "United. Focused. Ready.",
    tag: "USL-26-SS-MTCH",
    environment: "Concrete pitch",
    image: "/assets/unusual-drop-jersey.png",
    alt: "Model wearing a red and black UNUSUAL jersey uniform.",
    position: "48% 30%",
    pieces: [
      {
        number: "01",
        ...
        price: 98,
        image: "/assets/unusual-drop-jersey.png",
        alt: "Red and black UNUSUAL jersey.",
        position: "50% 30%",
      },
      {
        number: "02",
      ...
      },
      {
        ...
        price: 88,
        image: "/assets/unusual-drop-accessories.png",
        alt: "Black utility crossbody bag.",
        position: "50% 34%",
      },
      {
        number: "04",
  ...
  },
  {
    ...
    slug: "off-grid",
    name: "Off Grid",
    signal: "Offline. Untraceable.",
    tag: "USL-26-SS-OFFGD",
    environment: "Back route",
    image: "/assets/unusual-drop-knit.png",
    alt: "Model wearing a washed charcoal UNUSUAL hoodie with technical styling.",
    position: "54% 32%",
    pieces: [
      {
        number: "01",
        ...
        price: 148,
        image: "/assets/unusual-drop-knit.png",
        alt: "Washed charcoal UNUSUAL hoodie.",
        position: "54% 32%",
      },
      {
        number: "02",
...
function BuildUniform() {
  ...
  const mode = uniformModes[activeMode];
  const selectedPiece = mode.pieces[activePiece];
  const total = mode.pieces.reduce((sum, piece) => sum + piece.price, 0);
  ...
  return (
    <section className="uniform-builder" id="uniform" aria-labelledby="uniform-heading">
      <div className="uniform-frame">
        <div className="uniform-copy">
          <div className="uniform-ruler" aria-hidden="true">
            <span>/// UNUSUAL SYSTEMS</span>
            <i />
          </div>
          <h2 id="uniform-heading">Build Your Uniform</h2>
          <p>Choose the signal. Build the uniform. Leave ordinary behind.</p>

          <div className="uniform-modes" role="tablist" aria-label="Uniform styling modes">
            {uniformModes.map((uniformMode, index) => (
              <button
                type="button"
                role="tab"
                aria-selected={activeMode === index}
                className={activeMode === index ? "is-active" : ""}
                key={uniformMode.slug}
                onClick={() => selectMode(index)}
              >
                <span>{uniformMode.code}</span>
                <strong>{uniformMode.name}</strong>
                <em>{uniformMode.signal}</em>
                <i aria-hidden="true" />
              </button>
            ))}
          </div>

          <dl className="uniform-specs" aria-label="Active uniform details">
            <div>
              <dt>Signal</dt>
              <dd>{mode.name}</dd>
            </div>
            <div>
              <dt>Environment</dt>
              <dd>{mode.environment}</dd>
            </div>
            <div>
              <dt>Pieces</dt>
              <dd>{String(mode.pieces.length).padStart(2, "0")}</dd>
            </div>
          </dl>
        </div>

        <div className="uniform-build" aria-label={`${mode.name} outfit builder`}>
          <div className="uniform-piece-strip" aria-label={`${mode.name} uniform pieces`}>
            {mode.pieces.map((piece, index) => (
              <button
                type="button"
                className={`uniform-piece ${activePiece === index ? "is-active" : ""}`}
                key={`${mode.slug}-${piece.number}`}
                onClick={() => setActivePiece(index)}
                style={{ "--piece-index": index }}
                aria-label={`Show ${piece.name}`}
              >
                <span>{piece.number}</span>
                <img src={piece.image} alt="" aria-hidden="true" style={{ objectPosition: piece.position }} />
                <strong>{piece.category}</strong>
              </button>
            ))}
          </div>

          <figure className="uniform-model-card" key={mode.slug}>
            <img src={mode.image} alt={mode.alt} style={{ objectPosition: mode.position }} />
            <span className="uniform-scan" aria-hidden="true" />
            <span className="uniform-corners" aria-hidden="true" />
            <figcaption>
              <span>UNUSUAL</span>
              <em>{mode.name} // Signal locked</em>
            </figcaption>
          </figure>

          <div className="uniform-active-piece" aria-live="polite">
            <span>{selectedPiece.number}</span>
            <strong>{selectedPiece.name}</strong>
            <em>{selectedPiece.category}</em>
          </div>
        </div>

        <aside className="uniform-cart" aria-label={`${mode.name} uniform product stack`}>
          <div className="uniform-cart-head">
            <span>Your uniform</span>
            <strong>{mode.pieces.length} items</strong>
          </div>

          <div className="uniform-stack">
            {mode.pieces.map((piece, index) => (
              <button
                type="button"
                className={activePiece === index ? "is-active" : ""}
                key={`${mode.slug}-stack-${piece.number}`}
                onClick={() => setActivePiece(index)}
              >
                <span>{piece.number}</span>
                <img src={piece.image} alt="" aria-hidden="true" style={{ objectPosition: piece.position }} />
          ...
          <div className="uniform-total">
            <span>Total</span>
            <strong>${total}</strong>
          </div>

          <a className="button uniform-shop" href="#shop">
            <span>Shop this uniform</span>
            <ArrowIcon />
          </a>

          <button className={`uniform-save ${isSaved ? "is-saved" : ""}`} type="button" onClick={() => setIsSaved(true)}>
            <span>{isSaved ? "Fit saved" : "Save fit"}</span>
            <i aria-hidden="true" />
          </button>

          <p>{mode.tag} // Built different</p>
        </aside>
      </div>
    </section>
  );
